<template>
    ...
    <div class="container">
        ...
        <div id="testData" class="frameContrast">
                ...
                <div style="width: 800px;height: 80px;padding-top: 30px;padding-left: 520px;background:#c7ddef">
...
<!--                    <el-button type="primary" @click="TestResult">测试<i class="el-icon-orange el-icon&#45;&#45;right"></i></el-button>-->
                    <el-dropdown split-button type="primary" @command="TestResult" style="margin-left: 30px">
                        测&emsp;试<i class="el-icon-orange el-icon--right"></i>
                        <el-dropdown-menu slot="dropdown" >
                            <el-dropdown-item command="a">TNN</el-dropdown-item>
                            <el-dropdown-item command="b">对比模型1</el-dropdown-item>
<!--                            <el-dropdown-item command="c">对比模型2</el-dropdown-item>-->
                        </el-dropdown-menu>
                    </el-dropdown>

                </div>

                <div class="h1title" style="float: left;margin-bottom: 20px">测试结果</div>
        ...
        <div id="testResultIndex" class="frameContrast">
                ...
                <div style="width: 400px;margin-left: 40px;padding-top: 20px">
                    <el-table :data="Result" stripe style="width: 100%">
                        <el-table-column
                            prop="index" label="TNN"
                            width="100">
                        </el-table-column>
                        <el-table-column prop="R2" label="R2" width="150">
                        </el-table-column>
                        <el-table-column prop="RMSE" label="RMSE" width="150">
                        </el-table-column>
                    </el-table>
                </div>
            <div style="width: 400px;margin-left: 40px;padding-top: 50px">
                <el-table :data="Result1" stripe style="width: 100%">
                    <el-table-column
                        prop="index" label="GABPNN"
                        width="100">
                    </el-table-column>
                    <el-table-column prop="R2" label="R2" width="150">
                    </el-table-column>
...
let CompareData=[
    //元数据
    [15713.91974,
        16015.02828, 15891.69147, 16218.16112, 16051.47614, 18342.42524, 17358.14431, 12261.25375, 16975.58267, 17423.11741,],
    [2942.392641,
        3013.518114, 3179.002699, 3119.361978, 3172.595804, 3013.220918, 2657.163266, 1370.768179, 3200.057552, 3066.926265,],
    [60.83314267,
        67.05818797,
        73.1904141,
        69.30692463,
...
export default {
    ...
    methods:{
        ...
        NewSample(){
            ...
            this.$http.get('http://localhost:8000/upload/testData/')
            .then(function(res){
        ...
        TestResult(command){
            ...
            if(command=="a")
            {
                let temp=0;
                //注意格式TNN
                // let temp=JSON.stringify({"预测数据":this.TableData});
                this.$http.get('http://localhost:8000/TNNPredict/',{temp})
                    .then(function(res) {
                        {
                        ...
                        let ss2 = this.option2.series;
                        let ss3 = this.option3.series;
                        let ss4 = this.option4.series;
                        let sss = [];
                        sss.push(ss2);
                        sss.push(ss3);
                        sss.push(ss4);
                        ...
                        let source6 = [];
                        LSTMres.forEach((item, j) => {
                            source4.push(item.F / 4);
                            source5.push(item.T);
                            source6.push(item.S * 40);
                        });
                        source4 = source4.slice(0, 10);
                        source5 = source5.slice(0, 10);
                        source6 = source6.slice(0, 10);
                        tempsource2.push(source4);
                        tempsource2.push(source5);
                        tempsource2.push(source6);


                        let zz = this.option1.dataset.source;
                        // console.log(zz);
                        // console.log(tempsource2);
                        ...
                        for (let i = 0; i < zz.length; i++) {
                            ...
                            if (i >= 1) {
                                ...
                                for (let wu = 0; wu < 10; wu++) {
                                    tempindex.push(tempsource2[i - 1][wu]);
                                }
                        ...
                        this.$http.get('http://localhost:8000/upload/predictModel/')
                            .then(function(res){
                                let yy=res.data;
                                let source=this.Result;
                                let start=yy.length/2;
                                for(let i=0;i<start;i++)
                                {
                                    this.Result[i].R2=yy[i].test_r2_score;
                                    this.Result[i].RMSE=yy[i].test_mse
                                }
                                console.log(source);
            ...
            else if (command=="b")
            {
                // let temp=JSON.stringify({"预测数据":this.TableData});
                let temp=0;
                this.$http.get('http://localhost:8000/GABPNNPredict/',{temp})
                    .then(function(res){
                        {let LSTMres=res.data;
                        ...
                        this.$http.get('http://localhost:8000/upload/predictModel/')
                            .then(function(res){
                                let yy=res.data;
                                let source=this.Result1;
                                let start=yy.length/2;
                                for(let i=start;i<yy.length;i++)
                                {
                                    // console.log(this.Result1[i-start].R2);
                                    // console.log(this.Result1[i-start].RMSE);
                                    // console.log(yy[i].test_r2_score);
                                    // console.log(yy[i].test_mse);
                                    this.Result1[i-start].R2=yy[i].test_r2_score;
                                    this.Result1[i-start].RMSE=yy[i].test_mse
        ...
        AfterCanvas(temp)
        {
            console.log(CompareData[4]);
            console.log("-------------------");
            for(let i=0;i<3;i++)
            {
                // let temps=CompareData[i+3];
                // CompareData[i+3]=temps.splice(0,10,tempsource[i]);
                CompareData[i+3]=temp[i];
            }
            console.log(CompareData[4]);
            console.log("-------------------");
        }
...
// {
//     let temp=JSON.stringify({"预测数据":this.TableData});
//     this.$http.post(this.baseURL+'api/TNN模型测试数据3',{temp})
//         .then(function(res){
//             this.Result=undefined;
//             this.Result=res.data.filter(l=>l.name="index");
//             CompareData[3]=res.data.filter(l=>l.name="result1")[0];
//             CompareData[4]=res.data.filter(l=>l.name="result2")[1];
//             CompareData[5]=res.data.filter(l=>l.name="result3")[2];
//         })
//         .catch()
//     {
//         console.error("后端获取结果数据失败")
...
// let some=this.option1.dataset.source;
// // console.log(some);
// for(let i=0;i<some.length;i++)
// {
//     // if(i==0) continue;
//     // let tempindex=some[i];
//     //
//     // console.log(tempsource[i-1]);
//     // console.log(tempindex);
//     // some[i]=tempindex.splice(1,10,tempsource[i-1]);
//     // console.log(tempindex);
// }
</script>
